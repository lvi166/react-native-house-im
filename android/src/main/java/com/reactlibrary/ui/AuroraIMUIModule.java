...
import com.reactlibrary.ui.messagelist.commons.BitmapLoader;
import com.reactlibrary.ui.model.RCTMessage;
import com.reactlibrary.ui.model.RCTUser;
...
//    }
    ...
    @ReactMethod
    public void updateMessage(ReadableArray messages) {
        RCTMessage[] rctMessages = new RCTMessage[messages.size()];
        for (int i = 0; i < messages.size(); i++) {
            RCTMessage rctMessage = configMessage(messages.getMap(i));
            rctMessages[i] = rctMessage;
        }
        EventBus.getDefault().post(new MessageEvent(rctMessages, ReactMsgListManager.RCT_UPDATE_MESSAGE_ACTION));
    ...
    public void removeMessage(String id) {
        MessageEvent event = new MessageEvent(id, ReactMsgListManager.RCT_REMOVE_MESSAGE_ACTION);
        EventBus.getDefault().post(event);
    }

    @ReactMethod
    public void removeAllMessage() {
        MessageEvent event = new MessageEvent("", ReactMsgListManager.RCT_REMOVE_ALL_MESSAGE_ACTION);
        EventBus.getDefault().post(event);
    }
